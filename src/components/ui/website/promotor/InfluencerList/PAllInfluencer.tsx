...
const PAllInfluencer = ({ allCreator }: any) => {
                  ...
                  {allCreator?.data?.data && allCreator?.data?.data?.map((row: any, i: number) => (
                    <TableRow key={i}>
                      ...
                      <TableCell className="text-right md:w-[50px]">
                        <div className=" flex items-center gap-3">
                          {row?.isFollowing ? <Button variant="outline" className="border border-primary text-primary bg-transparent"
                          >Following
                          </Button> :
